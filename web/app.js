...
if (inputPrazo) {
  ...
  inputPrazo.addEventListener('blur', () => {
    if (inputPrazo.value) {
      ...
      target.setMonth(target.getMonth() + parseInt(inputPrazo.value));
...
if (formEl) {
  formEl.addEventListener('submit', async (e) => {
    ...
    if (lastEdited === 'data' && inputFuture.value) {
      ...
      payload.prazoMeses = parseInt(inputPrazo.value) || 0;
    ...
    try {
      ...
      document.getElementById('summary').innerHTML = `
        <div class="metric-card"><div style="color:var(--muted)">Investido</div><div class="metric-value">${brl(data.totalInvestido)}</div></div>
        <div class="metric-card"><div style="color:var(--muted)">Rend. Bruto</div><div class="metric-value">${brl(data.totalJurosBruto)}</div></div>
        <div class="metric-card"><div style="color:var(--muted)">Total IRPF</div><div class="metric-value" style="color:var(--danger)">${brl(data.totalIrpf)}</div></div>
        <div class="metric-card" style="border-color:var(--accent)"><div style="color:var(--accent)">Líquido Final</div><div class="metric-value">${brl(data.resultadoLiquido)}</div></div>
      `;
...
function exportPDF() {
  ...
  doc.autoTable({ html: '#tabela', startY: 25, theme: 'grid' });
  doc.save('simulacao.pdf');
}

function exportCSV() {
  let csv = "Mes;Saldo Inicial;Aporte;Rendimento Bruto;IRPF;Saldo Liquido\n";
  const rows = document.querySelectorAll("#tabela tbody tr");
  rows.forEach(row => {
    const cols = row.querySelectorAll("td");
    const rowData = Array.from(cols).map(c => c.innerText.replace(/[R$\s.]/g, '').replace(',', '.'));
    csv += rowData.join(";") + "\n";
  });
